fetch_aes_ggplotBuild=function(p,geom_list){
  ...
  if(class(p$data)!='waiver') mapping_class=lapply(train_map(p),function(m) class(p$data[,as.character(m)]))
  ...
  l=lapply(data,function(x) {
    ...
    class.temp=sapply(names(x$data)[names(x$data)%in%x$nm],function(item) {
      class(as.data.frame(pData)[,as.character(c(p$mapping[item][[1]],cl$var[cl$layer==x$layer&cl$aes==item]))])
    })
