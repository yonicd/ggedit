#' @title themeFetch
#' @description returns a nested list of a gg theme object, for ggedit gadget or module.
#' @param a gg theme
#' @return list
#' @keywords internal
themeFetch <- function(a=theme_get()){
  ...
  for( i in 1:length(obj.names) ){
    ...
    if( obj.names[i]%in%c('legend.position','legend.justification') ) 
      obj$call <- gsub('legend.','',obj.names[i])
