  ...
  plotHeight <- get(".plotHeight", envir = .ggeditEnv)
    ...
    server <- function(input, output, session) {
        ...
        shiny::observe({
          ...
          TEMPLIST$obj.new <- TEMPLIST$objList.new[[plotIdx()]]
          ...
          for(item in names(TEMPLIST$obj.Elems[[layer.idx]])){
            ...
            if( numElem[item]==1 ) {
              ...
              if( TEMPLIST$obj.Elems[[layer.idx]][[item]][['class']][[1]]=='numeric' ){
                ...
                if( !grepl('size|shape|linetype',item) ) 
                  vals <- paste0("'",vals,"'")
...
#Theme----
        ...
        update.ThemeGrid <- shiny::eventReactive(input$SetThemeGrid,{
          ...
          if( length(TEMPLIST$p.now$theme)>0 ) 
            theme.now <- theme.now+TEMPLIST$p.now$theme
...
#Render Plot----
        ...
        shiny::observeEvent(input$done, {
          ...
          if( verbose ) 
            ggeditOut$UpdatedLayerCalls <- lapply(TEMPLIST$objList.new,function(p) lapply(p$layer,function(item) cloneLayer(l = item,verbose = T,showDefaults = showDefaults)))
        ...
        output$SimPrint <- shiny::renderUI({
          ...
          if( input$viewVerbose%%2==1 ){
            ...
            if( Sys.info()[1] == "Windows" ){
              ...
              shinyAce::aceEditor(outputId = "codeout",
                                  value=toace,
                                  mode = "r", 
                                  theme = "chrome", 
                                  height = "100px", 
