...
ggEdit <- function(input, output, session,obj,verbose=TRUE,showDefaults=FALSE,width='auto',height='auto') {
              ...
              do.call(shiny::tabsetPanel,
                      unlist(lapply(1:length(TEMPLIST$obj.theme[[plotIdx()]]),FUN = function(j){
                        ...
                        if( themeListDepth(TEMPLIST$obj.theme[[plotIdx()]][j])>2 ){
                          ...
                          unlist(lapply(j, function(i) {themeMakePanel(TEMPLIST$obj.theme[[plotIdx()]][i],session=session)}),recursive = FALSE)}
  ...
  shiny::observeEvent(input$SetThemeGrid,{
    ...
    output$Plot <- shiny::renderPlot({as.ggedit(pList.out)},width=width,height=height)
  ...
  Out <- shiny::eventReactive({c(input$sendTheme,input$sendElem)},{
