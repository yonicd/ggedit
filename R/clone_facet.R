...
cloneFacet<-function(obj,verbose=FALSE){
  ...
  facet_p<-switch(class(obj)[1],
         FacetWrap={
           ...
           facet_wrap_p$scales<-switch(sum(which(as.logical(c(facet_wrap_p$free))))+1,{'fixed'},{'free_x'},{'free_y'},{'free'})
         ...
         FacetGrid={
           ...
           facet_grid_p$facets<-sprintf('%s~%s',paste0(switch((length(facet_grid_p$rows)>0)+1,'.',facet_grid_p$rows),collapse='+'),
                                        paste0(switch((length(facet_grid_p$cols)>0)+1,'.',facet_grid_p$cols),collapse = '+'))
           facet_grid_p$scales<-switch(sum(which(as.logical(facet_grid_p$free)))+1,{'fixed'},{'free_x'},{'free_y'},{'free'})
           facet_grid_p$space<-switch(sum(which(as.logical(facet_grid_p$space_free)))+1,{'fixed'},{'free_x'},{'free_y'},{'free'})
